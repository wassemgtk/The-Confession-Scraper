...
class _iterinfo(object):
    ...
    def rebuild(self, year, month):
        ...
        if (rr._bynweekday and (month != self.lastmonth or # Access to member 'lastmonth' before its definition line 1228
                                year != self.lastyear)): # Access to member 'lastmonth' before its definition line 1228 (An example for such a situation is an instance attribute in init() that is read before it was initialized. This includes usage of the += operator on an uninitialized instance attribute.)
            ranges = []
            if rr._freq == YEARLY:
                if rr._bymonth:
